...
function formatNombres(x) {
 return Highcharts.numberFormat(x, 0, '.', ',') + ' times';
}

Highcharts.chart('container', {
 title: {
   text: 'Initial example problem',
   margin: 35
 },
 xAxis: [{
   title: {
   ...
   title: {
     text: 'Example of data on x-axis'
   },
   accessibility: {
     description: 'Example of data on x-axis',
     rangeDescription: ''
   },
   alignTicks: false,
   opposite: false
   ...
   series: {
     descriptionFormatter: function(series) {
       return series.type === 'line' ?
         ...
         series.name + ' histogram with ' +
         series.points.length + ' bars.';
     }
   },
   point: {
     pointValueSuffix: ' times'
   },
   keyboardNavigation: {
     seriesNavigation: {
       mode: 'serialize'
